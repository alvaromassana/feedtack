...
const enShadow = (pg, sel) => pg.evaluate(s => {
  const n = document.querySelector('#tack-host').shadowRoot.querySelector(s);
...
async function comentar(pg, texto, selObjetivo) {
  ...
  // nombre y enviar
  await pg.locator('#tack-host').evaluate(h => {
...
let estado = await sh(p, () => {
  ...
  return {
    ...
    marcables: window.Tack.estado().comentarios
...
await sh(p, id => window.Tack.verComentario(id), ID1);
await p.waitForTimeout(700);
const puedeEditar = await sh(p, () => !![...document.querySelector('#tack-host').shadowRoot.querySelectorAll('button')]
  .find(x => x.textContent.trim() === 'Editar'));
if (!puedeEditar) fallos.push('el autor no ve el boton de editar en su propio comentario');
else {
  await sh(p, () => [...document.querySelector('#tack-host').shadowRoot.querySelectorAll('button')]
    .find(x => x.textContent.trim() === 'Editar').click());
  await p.waitForTimeout(400);
  await p.locator('#tack-host').evaluate(h => {
...
const veResolver = await sh(p, () => !![...document.querySelector('#tack-host').shadowRoot.querySelectorAll('button')]
...
await pa.goto(BASE + '/?tack_admin=' + encodeURIComponent(CLAVE), { waitUntil: 'networkidle' });
await listo(pa);
await sh(pa, id => window.Tack.verComentario(id), ID1);
await pa.waitForTimeout(800);
const veResolverAdmin = await sh(pa, () => !![...document.querySelector('#tack-host').shadowRoot.querySelectorAll('button')]
  .find(x => x.textContent.includes('Marcar como resuelto')));
if (!veResolverAdmin) fallos.push('con clave de administracion NO aparece el boton de resolver');
else {
  await sh(pa, () => [...document.querySelector('#tack-host').shadowRoot.querySelectorAll('button')]
    .find(x => x.textContent.includes('Marcar como resuelto')).click());
  await pa.waitForTimeout(3500);
  const chapa = await sh(pa, () => document.querySelector('#tack-host').shadowRoot.querySelector('.chapa')?.textContent);
...
const veConfirmar = await sh(p, () => !![...document.querySelector('#tack-host').shadowRoot.querySelectorAll('button')]
...
if (!veConfirmar) fallos.push('el cliente no ve la opcion de confirmar en un comentario resuelto');
  ...
  const chapa = await sh(p, () => document.querySelector('#tack-host').shadowRoot.querySelector('.chapa')?.textContent);
...
const cuenta = await sh(p, () => {
  const s = document.querySelector('#tack-host').shadowRoot;
  const est = window.Tack.estado().comentarios;
  return {
